    ...
    internal class TestFixture_02_Assistants : AbstractTestFixture
    {
        ...
        public async Task Test_01_Assistants()
        {
            ...
            try
            {
                ...
                var assistant = await HuoshanAIClient.AssistantsEndpoint.CreateAssistantAsync(request);
                ...
                try
                {
                    ...
                    var assistantsList = await HuoshanAIClient.AssistantsEndpoint.ListAssistantsAsync();
                    Assert.IsNotNull(assistantsList);
                    Assert.IsNotEmpty(assistantsList.Items);

                    foreach (var asst in assistantsList.Items)
                    {
                        var retrievedAsst = await HuoshanAIClient.AssistantsEndpoint.RetrieveAssistantAsync(asst);
