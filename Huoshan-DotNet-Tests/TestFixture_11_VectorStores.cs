...
namespace HuoshanAI.Tests
{
    internal class TestFixture_11_VectorStores : AbstractTestFixture
    {
        ...
        {
            Assert.IsNotNull(HuoshanAIClient.VectorStoresEndpoint);
            ...
            try
            {
                try
                {
                    file = await HuoshanAIClient.FilesEndpoint.UploadFileAsync(testFilePath, FilePurpose.Assistants);
                }
                finally
                {
                ...
                try
                {
                    ...
                    vectorStore = await HuoshanAIClient.VectorStoresEndpoint.CreateVectorStoreAsync(createVectorStoreRequest);
                    ...
                    // list vector stores
                    var vectorStores = await HuoshanAIClient.VectorStoresEndpoint.ListVectorStoresAsync();
                    ...
                    var modifiedVectorStore = await HuoshanAIClient.VectorStoresEndpoint.ModifyVectorStoreAsync(vectorStore, metadata: metadata);
                    Assert.IsNotNull(modifiedVectorStore);
                    Assert.AreEqual(vectorStore.Id, modifiedVectorStore.Id);

                    // retrieve vector store
                    var retrievedVectorStore = await HuoshanAIClient.VectorStoresEndpoint.GetVectorStoreAsync(vectorStore);
                    ...
                    try
                    {
                        ...
                        vectorStoreFile = await HuoshanAIClient.VectorStoresEndpoint.CreateVectorStoreFileAsync(vectorStore, file, new ChunkingStrategy(ChunkingStrategyType.Static));
                        Assert.IsNotNull(vectorStoreFile);

                        // list vector store files
                        var vectorStoreFiles = await HuoshanAIClient.VectorStoresEndpoint.ListVectorStoreFilesAsync(vectorStore);
                        Assert.IsNotNull(vectorStoreFiles);
                        Assert.IsNotEmpty(vectorStoreFiles.Items);

                        // retrieve vector store file
                        var retrievedVectorStoreFile = await HuoshanAIClient.VectorStoresEndpoint.GetVectorStoreFileAsync(vectorStore, vectorStoreFile);
                    ...
                    finally
                    {
                        if (vectorStoreFile != null)
                        {
                            ...
                            var deletedVectorStoreFile = await HuoshanAIClient.VectorStoresEndpoint.DeleteVectorStoreFileAsync(vectorStore, vectorStoreFile);
                ...
                finally
                {
                    ...
                    {
                        // delete vector store
                        var deletedVectorStore = await HuoshanAIClient.VectorStoresEndpoint.DeleteVectorStoreAsync(vectorStore);
        ...
        public async Task Test_02_VectorStores_BatchFiles()
        {
            ...
            try
            {
                try
                {
                    ...
                    {
                        Task.Run(async () => files.Add(await HuoshanAIClient.FilesEndpoint.UploadFileAsync(testFilePath1, FilePurpose.Assistants))),
                        Task.Run(async () => files.Add(await HuoshanAIClient.FilesEndpoint.UploadFileAsync(testFilePath2, FilePurpose.Assistants)))
                    };
                ...
                try
                {
                    ...
                    var vectorStoreFileBatch = await HuoshanAIClient.VectorStoresEndpoint.CreateVectorStoreFileBatchAsync(vectorStore, files.ToList());
                    Assert.IsNotNull(vectorStoreFileBatch);

                    // cancel vector store batch
                    var cancelledVectorStoreFileBatch = await HuoshanAIClient.VectorStoresEndpoint.CancelVectorStoreFileBatchAsync(vectorStore, vectorStoreFileBatch);
                    Assert.IsNotNull(cancelledVectorStoreFileBatch);
                    Assert.IsTrue(cancelledVectorStoreFileBatch);

                    // create vector store batch
                    vectorStoreFileBatch = await HuoshanAIClient.VectorStoresEndpoint.CreateVectorStoreFileBatchAsync(vectorStore, files.ToList());
                    Assert.IsNotNull(vectorStoreFileBatch);

                    // currently no way to list vector store batches
                    //var vectorStoreFileBatches = await HuoshanAIClient.VectorStoresEndpoint.ListVectorStoreFileBatchesAsync(vectorStore);
                    ...
                    var vectorStoreBatchFiles = await HuoshanAIClient.VectorStoresEndpoint.ListVectorStoreBatchFilesAsync(vectorStore, vectorStoreFileBatch);
                    ...
                    foreach (var file in vectorStoreBatchFiles.Items)
                    {
                        ...
                        var retrievedVectorStoreBatchFile = await HuoshanAIClient.VectorStoresEndpoint.GetVectorStoreFileAsync(vectorStore, file);
            ...
            finally
            {
                if (!files.IsEmpty)
                {
                    var deleteTasks = files.Select(file => HuoshanAIClient.FilesEndpoint.DeleteFileAsync(file)).ToList();
