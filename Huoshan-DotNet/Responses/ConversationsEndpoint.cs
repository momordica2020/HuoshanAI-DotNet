...
{
    public sealed class ConversationsEndpoint : HuoshanAIBaseEndpoint
    {
        public ConversationsEndpoint(HuoshanClient client) : base(client) { }
        ...
        {
            var payload = JsonSerializer.Serialize(request, HuoshanClient.JsonSerializationOptions).ToJsonStringContent();
        ...
        public async Task<Conversation> UpdateConversationAsync(string conversationId, IReadOnlyDictionary<string, string> metadata, CancellationToken cancellationToken = default)
        {
            ...
            var payload = JsonSerializer.Serialize(new { metadata }, HuoshanClient.JsonSerializationOptions).ToJsonStringContent();
        ...
        public async Task<ListResponse<IResponseItem>> CreateConversationItemsAsync(string conversationId, IEnumerable<IResponseItem> items, string[] include = null, CancellationToken cancellationToken = default)
        {
            ...
            var payload = JsonSerializer.Serialize(new { items }, HuoshanClient.JsonSerializationOptions).ToJsonStringContent();
