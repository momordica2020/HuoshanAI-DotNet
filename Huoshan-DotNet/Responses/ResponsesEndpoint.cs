...
{
    /// <summary>
    /// HuoshanAI's most advanced interface for generating model responses.
    ...
    /// </summary>
    public sealed class ResponsesEndpoint : HuoshanAIBaseEndpoint
    {
        public ResponsesEndpoint(HuoshanClient client) : base(client) { }
        ...
        public async Task<(T, Response)> CreateModelResponseAsync<T>(CreateResponseRequest request, Func<string, IServerSentEvent, Task> streamEventHandler = null, CancellationToken cancellationToken = default)
        {
            ...
            if (lastItem is Message messageItem)
            {
                ...
                output = JsonSerializer.Deserialize<T>(messageItem.ToString(), HuoshanClient.JsonSerializationOptions);
            }
