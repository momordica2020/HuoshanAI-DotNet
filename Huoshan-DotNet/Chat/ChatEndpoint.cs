...
namespace HuoshanAI.Chat
{
    ...
    /// </summary>
    public sealed class ChatEndpoint : HuoshanAIBaseEndpoint
    {
        /// <inheritdoc />
        public ChatEndpoint(HuoshanClient client) : base(client) { }
        ...
        {
            using var payload = JsonSerializer.Serialize(chatRequest, HuoshanClient.JsonSerializationOptions).ToJsonStringContent();
        ...
        public async Task<(T, ChatResponse)> GetCompletionAsync<T>(ChatRequest chatRequest, CancellationToken cancellationToken = default)
        {
            ...
            var output = JsonSerializer.Deserialize<T>(response.FirstChoice, HuoshanClient.JsonSerializationOptions);
        ...
        public async IAsyncEnumerable<ChatResponse> StreamCompletionEnumerableAsync(ChatRequest chatRequest, bool streamUsage = false, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            while (await reader.ReadLineAsync(cancellationToken) is { } streamData)
            {
                ...
                if (responseStream != null)
                {
                    ...
                    try
                    {
                        data = JsonNode.Parse(eventData)?.ToJsonString(HuoshanClient.JsonSerializationOptions);
                    }
                    catch
                    {
                        data = $"{{{eventData}}}";
