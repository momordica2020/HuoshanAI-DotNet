...
{
    /// <summary>
    /// <see href="https://platform.HuoshanAI.com/docs/guides/function-calling"/>
    /// </summary>
    public sealed class Function : ITool
    {
        ...
        /// <see href="https://platform.HuoshanAI.com/docs/api-reference/assistants/docs/guides/function-calling"/>
        /// </param>
        ...
        public string Invoke()
        {
            try
            {
                ...
                return JsonSerializer.Serialize(new { result }, HuoshanClient.JsonSerializationOptions);
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
                return JsonSerializer.Serialize(new { error = e.Message }, HuoshanClient.JsonSerializationOptions);
            }
            finally
            {
                Arguments = null;
        ...
        private (Function function, object[] invokeArgs) ValidateFunctionArguments(CancellationToken cancellationToken = default)
        {
            ...
            var requestedArgs = Arguments != null
                ? JsonSerializer.Deserialize<Dictionary<string, object>>(Arguments.ToString(), HuoshanClient.JsonSerializationOptions)
                : new();
            ...
            for (var i = 0; i < methodParams.Length; i++)
            {
                ...
                if (requestedArgs.TryGetValue(parameter.Name, out var value))
                {
                    ...
                    {
                        invokeArgs[i] = JsonSerializer.Deserialize(element.GetRawText(), parameter.ParameterType, HuoshanClient.JsonSerializationOptions);
                    }
                    else
                    {
