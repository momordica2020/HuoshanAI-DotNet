...
namespace HuoshanAI
{
    public abstract class HuoshanAIBaseEndpoint
    {
        protected HuoshanAIBaseEndpoint(HuoshanClient client) => this.client = client;

        // ReSharper disable once InconsistentNaming
        protected readonly HuoshanClient client;
        ...
        private string GetEndpoint(string baseUrlFormat, string endpoint = "", Dictionary<string, string> queryParameters = null)
        {
            ...
            if (client.Settings.IsAzureHuoshanAI && IsAzureDeployment == true)
            {
                if (string.IsNullOrWhiteSpace(client.Settings.DeploymentId))
                {
                    throw new InvalidOperationException("Deployment ID must be provided for Azure HuoshanAI endpoints.");
                }
