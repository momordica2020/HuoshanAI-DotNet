...
namespace HuoshanAI.Audio
{
    /// <summary>
    /// Transforms audio into text.<br/>
    /// <see href="https://platform.HuoshanAI.com/docs/api-reference/audio"/>
    /// </summary>
    public sealed class AudioEndpoint : HuoshanAIBaseEndpoint
    {
        /// <inheritdoc />
        public AudioEndpoint(HuoshanClient client) : base(client) { }
            ...
            try
            {
                ...
                if (request.ChunkingStrategy != null)
                {
                    var stringContent = request.ChunkingStrategy.Type == "auto"
                        ? "auto"
                        : JsonSerializer.Serialize(request.ChunkingStrategy, HuoshanClient.JsonSerializationOptions);
