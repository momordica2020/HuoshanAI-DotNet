...
{
    internal sealed class ResponseTextResponseObjectConverter : JsonConverter<TextResponseFormatConfiguration>
    {
        public override TextResponseFormatConfiguration Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            try
            {
                    ...
                    typeProperty.ValueKind == JsonValueKind.String)
                {
                    ...
                    {
                        if (!jsonObject.TryGetProperty("schema", out var schemaProperty) ||
